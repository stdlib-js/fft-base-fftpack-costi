...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof costi, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 4', function test( t ) {
	t.strictEqual( costi.length, 4, 'returns expected value' );
	t.end();
});
...
tape( 'the function returns a reference to the workspace array', function test( t ) {
	...
	out = costi( N, workspace, 1, 0 );

	t.strictEqual( out, workspace, 'same reference' );
	t.end();
});

tape( 'the function correctly initializes sine and cosine values (small sequence lengths)', function test( t ) {
	var workspace;
	var expected;
	var lengths;
	var offsets;
...
tape( 'the function correctly initializes sine and cosine values (medium sequence lengths)', function test( t ) {
	var workspace;
	var expected;
	var lengths;
	var offsets;
...
tape( 'the function correctly initializes sine and cosine values (large sequence lengths)', function test( t ) {
	var workspace;
	var expected;
	var lengths;
	var offsets;
...
tape( 'the function correctly handles stride and offset parameters', function test( t ) {
	...
	costi( N, workspace, stride, offset );
	...
	costi( N, expected, 1, 0 );

	for ( i = 0; i < N-1; i++ ) {
		t.strictEqual( workspace[ offset + ( ( (2*N)-1+i ) * stride ) ], expected[ (2*N)-1+i ], 'returns expected value' );
	}
	t.end();
});
